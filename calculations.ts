...
export function calculateTotals() {
    ...
    // 1. Calculate Single Unit Cost (Standard + Custom Items)
    ...
    const singleUnitCost = standardCost + customCost;
    
    // 2. Apply Markup to Single Unit
    ...
    const singleUnitPriceWithMarkup = singleUnitCost * (1 + markupValue / 100);

    // 3. Calculate Total for ALL Units
    const totalBasePrice = singleUnitPriceWithMarkup * state.globalQuantity;
    ...
    if (state.selectedDiscountId !== 'none') {
        ...
        if (selectedTier) {
            ...
            appliedDiscountLabel = selectedTier.threshold > 0 
                ? `满 ${selectedTier.threshold} 台, 打 ${selectedTier.rate} 折`
                : `固定折扣: ${selectedTier.rate} 折`;
        }
    }

    // 5. Final Calculation: Total * Rate - SpecialDiscount
    let finalPrice = totalBasePrice * appliedRate - state.specialDiscount;
...
export function getFinalConfigText() {
    ...
    if (state.globalQuantity > 1) {
        return `[共 ${state.globalQuantity} 台] ${configStr}`;
    }
    return configStr;
}
